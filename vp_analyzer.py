...
class DiscardValue(object):
    ...
    def pair_jqka(self):
        # nothing held
        if self.held_r_cnts == []:
            draw5 = self._draw_for_ranks(
                gsize=2, cnt_held_only=False, pairing_jqka=True
            )
            return draw5
        # most common card is a singleton
        elif self.held_r_cnts[0][1] == 1:
            draws = self._draw_for_ranks(
                gsize=2, cnt_held_only=False, pairing_jqka=True
            )
            return draws
    ...
    def full_house(self):
        ...
        # most common card is a singleton
            ...
            if self.draw_cnt == 2:
                draw_pair = self._draw_for_ranks(gsize=2, draw_cnt=2, draw_only=True)
                return draw_pair
...
if __name__ == "__main__":
    print(
        HandAnalyzer("qd9c8d5c2c").analyze(
